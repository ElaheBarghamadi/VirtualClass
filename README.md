...
**آزمون و تکلیف**
- **آزمون زندهٔ چهارگزینه‌ای**: ساخت آزمون در صفحهٔ مدیریت، شروع از تب «آزمون» داخل جلسه؛ پرسش‌ها یکی‌یکی در مودال ساده برای دانش‌آموزان (گزینهٔ صحیح هرگز به کلاینت ارسال نمی‌شود)، شمارندهٔ زندهٔ پاسخ‌ها برای میزبان، کارنامهٔ پایان آزمون (نمرهٔ هر فرد + جدول میزبان) و بایگانی نتایج هر اجرا
- **تکلیف و نمره**: تعریف تکلیف با شرح/مهلت/بارم، ارسال پاسخ متنی یا فایل (با همان اعتبارسنجی فایل‌های کلاس)، ویرایش تا مهلت، تصحیح با نمره و بازخورد؛ دانش‌آموز نمرهٔ خود را در همان صفحه می‌بیند
- ورود وسط آزمون: شرکت‌کنندهٔ تازه‌وارد آزمون در جریان را از اسنپ‌شات WS دریافت می‌کند

**ضبط و دسترسی**
- **ضبط جلسه**: دکمهٔ ضبط برای میزبان (صدا + تصویر اشتراک صفحه یا دوربین) با تایمر؛ فایل webm در پایان **روی دستگاه میزبان** ذخیره می‌شود — چیزی به سرور آپلود نمی‌شود
- **ممنوعیت مهمان بر اساس IP**: مهمانی که میزبان او را با ممنوعیت موقت خارج کند، تا پایان مهلت با همان IP نمی‌تواند دوباره وارد شود
- **موبایل**: اتاق روی ۳۹۰px هم کامل کار می‌کند — پنل کنار به کشوی تمام‌صفحه تبدیل می‌شود، تب‌ها اسکرول افقی دارند، مودال آزمون تمام‌عرض است
- **داشبورد**: جلسات زمان‌بندی‌شدهٔ کلاس‌هایی که عضو آن‌ها هستید + فهرست «کلاس‌هایی که عضو آن هستم» با دسترسی مستقیم به تکالیف
...
سوئیت `tests_e2e/run_e2e.py` با دو مرورگر واقعی (مالک + دانش‌آموز) ۳۵ سناریو
را می‌سنجد: گفتگوی زنده، بالا بردن دست، force-mute و آزادسازی
دستگاه، ارتقای نقش با رفرش خودکار، چرخهٔ کامل فایل‌ها (آپلود/ارائه/دانلود/حذف)،
ارائهٔ Office با تبدیل PDF سمت سرور، چرخهٔ کامل آزمون زنده (ساخت، شروع،
پاسخ، کارنامه)، تختهٔ سفید، دوربین + PiP، اسموک نمای موبایل ۳۹۰×۸۴۴ و نبود خطای JS.
...
| آزمون | شروع/پایان فقط میزبان، پاسخ مهمان/دانش‌آموز، اعتبارسنجی گزینه و پرسش، رد پاسخ پس از پایان، upsert تغییر پاسخ، نمره‌دهی، عدم نشت گزینهٔ صحیح در payload، رد آزمون کلاس دیگر، ساخت پرسش توسط مالک و رد غیرمالک |
| تکلیف | ساخت/حذف توسط مالک، رد ساخت توسط دانش‌آموز، ۴۰۴ برای غریبه، ارسال/ویرایش پاسخ، رد ارسال خالی و پس از مهلت، تصحیح + رد نمرهٔ خارج بازه، قفل شدن پاسخ تصحیح‌شده، رد تصحیح توسط دانش‌آموز |
| ممنوعیت مهمان | ثبت IP هنگام ورود، مسدود شدن همان IP در دورهٔ ممنوعیت، آزادی IP دیگر و پایان مهلت |
