...
**صفحه‌بندی:** هر عملیات با `page` برچسب می‌خورد؛ دکمهٔ «＋» یک عملیات `page_add` broadcast می‌کند تا شمارهٔ صفحات برای همه یکسان بماند. ناوبری صفحات محلی است (هر کاربر می‌تواند صفحهٔ خودش را ببیند) و همهٔ صفحات به‌طور خودکار سمت سرور **ذخیره** می‌شوند؛ دکمهٔ 💾 صفحهٔ جاری را به‌صورت PNG دانلود می‌کند.
